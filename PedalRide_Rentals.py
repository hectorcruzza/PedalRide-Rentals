...
class PedalRide_Rentals: # Creación de la clase.
    def __init__(self, df: pd.DataFrame, df_reservaciones: pd.DataFrame): # Función de inicialización.
        self.fecha_actual_datetime = dt.date.today()
        self.fecha_actual = self.fecha_actual_datetime.strftime("%d/%m/%Y")
        self.hora_actual_datetime = dt.datetime.now()
        self.hora_actual = self.hora_actual_datetime.strftime("%H:%M")
    ...
    def opciones_post_crear_cuenta(self):
        print("- Seleccione una de las siguientes opciones con base en su número: \n") # Impresión de un mensaje.
        if not self.inicio_sesion_exitoso:
    ...
    def mi_perfil(self):
        print("\nSección: Me.") # Impresión de un mensaje.
    ...
    def proceso_hacer_reservaciones(self):
        ...
        self.hora_actual = self.hora_actual_datetime.strftime("%H:%M")
        print(f"\n| Fecha: {self.fecha_actual} - Hora: {self.hora_actual} |\n")
        dia_actual = self.fecha_actual_datetime.day
        mes_actual = self.fecha_actual_datetime.month
        año_actual = self.fecha_actual_datetime.year
        horas_actual = self.hora_actual_datetime.hour
        minutos_actual = self.hora_actual_datetime.minute
        ...
        while True:
            ...
            and (fecha_reserva_datetime > self.fecha_actual_datetime or horas_anticipacion >= 1):
        ...
        while h < len(self.bicicletas) and not self.bicicletas_disponibles:
            ...
            l = 0
            while l < len(self.df_reservaciones) and self.bicicletas_disponibles:
                if self.bicicletas[h] == self.df_reservaciones["Número de Bicicleta"].iloc[l]:
                    container_fecha_reserva_datetime = dt.datetime.strptime(self.df_reservaciones["Fecha de la Reservación"].iloc[l], "%d/%m/%Y").date()
                    ...
                    if container_año_reserva == año_reserva:
                        if container_mes_reserva == mes_reserva:
                            if container_dia_reserva == dia_reserva:
                                container_hora_inicio = dt.datetime.strptime(self.df_reservaciones["Hora de Inicio"].iloc[l], "%H:%M").hour
                                container_hora_termino = dt.datetime.strptime(self.df_reservaciones["Hora de Término"].iloc[l], "%H:%M").hour
        ...
        if self.bicicletas_disponibles:
            ...
            for a in range(len(self.bicicletas)):
                ...
                b = 0
                while b < len(self.df_reservaciones) and bicicleta_disponible:
                    if self.bicicletas[a] == self.df_reservaciones["Número de Bicicleta"].iloc[b]:
                        container_fecha_reserva_datetime = dt.datetime.strptime(self.df_reservaciones["Fecha de la Reservación"].iloc[b], "%d/%m/%Y").date()
                        container_dia_reserva = container_fecha_reserva_datetime.day
                        container_mes_reserva = container_fecha_reserva_datetime.month
                        container_año_reserva = container_fecha_reserva_datetime.year
                        if container_año_reserva == año_reserva:
                            if container_mes_reserva == mes_reserva:
                                if container_dia_reserva == dia_reserva:
                                    container_hora_inicio = dt.datetime.strptime(self.df_reservaciones["Hora de Inicio"].iloc[b], "%H:%M").hour
                                    container_hora_termino = dt.datetime.strptime(self.df_reservaciones["Hora de Término"].iloc[b], "%H:%M").hour
                                    if horas_inicio_reserva < container_hora_termino and horas_termino_reserva > container_hora_inicio:
            ...
            if respuesta_usuario_precio == "Sí":
                ...
                numero_reservacion = len(self.df_reservaciones) + 1
                ...
                self.df_reservaciones = pd.read_csv("reservaciones.csv", dtype = str)
...
def main(): # Función main.
    ...
    df_reservaciones = pd.read_csv("reservaciones.csv", dtype = str)
